          ...
          <div className="p-4 p-md-5 d-flex flex-column align-items-center">
            {loading ? (
              ...
              <>
                ...
                <h1 className="font-playfair fw-bold fs-3 mb-2">Payment Confirmed!</h1>
                <p className="text-muted mb-4">
                  Your payment has been securely processed by Stripe.{' '}
                  {orderId && <span>Your order ID is <strong>{orderId}</strong>.</span>}
                  {' '}The Khata ledger has been successfully updated.
                </p>
                <Link to="/tracking" className="w-100 mt-2">
                  <button className="btn btn-accent btn-lg w-100">Track My Order</button>
                </Link>
                <Link to="/" className="w-100 mt-2">
                  <button className="btn btn-link text-muted w-100 text-decoration-none">Return to Home</button>
                </Link>
              </>
            )}
          </div>
