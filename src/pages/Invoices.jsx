    ...
    return (
        <Layout>
            <div className="container py-5 text-center">
                <h5>Please login to view your invoices.</h5>
            </div>
        </Layout>
    );
  }
      ...
      <div className="container py-5" dir={isUrdu ? 'rtl' : 'ltr'}>
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
          ...
          <div className="d-flex flex-wrap gap-2">
            {['All', 'unpaid', 'paid', 'overdue'].map(f => (
              <button 
                key={f}
                className={`btn btn-sm ${filter === f ? 'btn-accent' : 'btn-outline-secondary'}`}
                onClick={() => setFilter(f)}
        ...
        {loading ? (
            ...
            <div className="text-center py-5 my-5 th-card-static">
                <div className="d-inline-flex align-items-center justify-content-center rounded-circle mb-3" style={{ width: 64, height: 64, background: '#f3f4f6' }}>
                    <FileText size={32} className="text-muted opacity-50" />
                </div>
                <h5 className="font-playfair fw-bold">{t('invoice.empty', 'No invoices found')}</h5>
                <p className="text-muted small mb-0">You don't have any invoices matching your filters.</p>
            </div>
        ) : (
                ...
                {filteredInvoices.map(inv => (
                    <div key={inv.id} className="col-12 col-md-6 col-lg-4">
                        <div className="th-card-static p-4 h-100 d-flex flex-column anim-scale-in">
                            ...
                            <div className="small text-muted mb-4 flex-grow-1 p-3 rounded" style={{ background: '#f9fafb' }}>
                            ...
                            <div className="d-flex gap-2 mt-auto">
                                <button className="btn btn-accent flex-grow-1 d-flex justify-content-center align-items-center gap-2" onClick={() => handleDownload(inv.invoice_number)}>
                                    <Download size={16} />
                                    {t('invoice.actions.download', 'Download PDF')}
                                </button>
                                <a href={`/tracking?orderId=${inv.order_id}`} className="btn btn-outline-secondary d-flex justify-content-center align-items-center" title={t('invoice.actions.viewOrder', 'View Order')}>
                                    <Eye size={18} />
                                </a>
                            </div>
                        </div>
